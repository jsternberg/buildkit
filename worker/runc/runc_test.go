...
func TestRuncWorker(t *testing.T) {
	...
	_, err = w.WorkerOpt.Executor.Run(ctx, "", nil, execMount(snap, true), nil, executor.ProcessInfo{Meta: meta, Stderr: &nopCloser{stderr}}, nil)
	...
	_, err = w.WorkerOpt.Executor.Run(ctx, "", nil, execMount(root, false), nil, executor.ProcessInfo{Meta: meta, Stderr: &nopCloser{stderr}}, nil)
	require.NoError(t, err)
...
func TestRuncWorkerNoProcessSandbox(t *testing.T) {
	...
	stderr := bytes.NewBuffer(nil)
	_, err = w.WorkerOpt.Executor.Run(ctx, "", nil, execMount(root, false), nil, executor.ProcessInfo{Meta: meta, Stdout: &nopCloser{stdout}, Stderr: &nopCloser{stderr}}, nil)
